...
def test_base_sklearn(datasets, target_column, n_learners, perc_var, decimal_threshold, class_constraints=None, plot=False, save_plot_dir="examples/", attribute=None, communities=False, class_flag=False):
    """
    ...
    if class_constraints == None:
        X_train_filtered = X_train
    else:
        X_train_filtered = X_train[y_train == class_constraints]
        
        if len(X_train_filtered) == 0:
            raise Exception("Wrong class specified!")

    # X_train_filtered = X_train[y_train == set(y_train).pop()]
    # X_train_filtered = X_train[10]
    ...
    # Extract DPG
    dot = get_dpg(X_train_filtered, features, rf_classifier, perc_var, decimal_threshold)
    ...
    return rf_classifier, df, df_dpg
